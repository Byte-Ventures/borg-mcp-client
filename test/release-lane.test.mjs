...
function githubReleaseBindingDeps({
  tagType = 'tag',
  tagMessage = 'borgmcp 3.14.0',
  live = { name: 'borgmcp', version: '3.14.0', integrity: RELEASE_INTEGRITY },
} = {}) {
  return {
    ...
    git: (args) => {
      ...
      if (command.includes(`show ${RELEASE_COMMIT}:docs/releases/3.14.0.md`)) return RELEASE_NOTES;
      if (command.includes('show -s')) return 'Merge pull request #443 from Byte-Ventures/release/3.14.0';
...
test('GitHub Release body frames tag, live package, and exact tagged notes', () => {
  ...
  assert.match(body, /borgmcp@3\.14\.0/);
  ...
  for (const { name, options, pattern } of [
    ...
    await t.test(name, async () => {
      await assert.rejects(
        () => createGithubRelease('3.14.0', githubReleaseBindingDeps(options)),
        pattern,
      );
    });
  }
...
test('GitHub Release creation fails closed when tagged release notes are missing or blank', async (t) => {
  for (const { name, notes, pattern } of [
    { name: 'missing', notes: null, pattern: /must contain docs\/releases\/3\.14\.0\.md/ },
    ...
    await t.test(name, async () => {
      ...
      await assert.rejects(() => createGithubRelease('3.14.0', deps), pattern);
    });
  }
});
...
test('GitHub Release creation gates on the live package and Release absence', async () => {
  ...
  const result = await createGithubRelease('3.14.0', {
    allowMissingToken: true,
    git: (args) => {
      const command = args.join(' ');
      if (command.includes('cat-file -t')) return 'tag';
      if (command.includes('rev-parse')) return commit;
      if (command.includes('for-each-ref')) return 'borgmcp 3.14.0';
      if (command.includes(`show ${commit}:docs/releases/3.14.0.md`)) return RELEASE_NOTES;
      throw new Error(`unexpected git command: ${command}`);
    },
    livePackage: async () => {
  ...
  assert.equal(created.tag_name, 'v3.14.0');
  assert.equal(created.name, 'borgmcp 3.14.0');
...
test('GitHub Release creation rejects invalid live package identity and integrity', async () => {
  await assert.rejects(
    () => createGithubRelease('3.14.0', githubReleaseBindingDeps({
      live: { name: 'other', version: '3.14.0', integrity: RELEASE_INTEGRITY },
    })),
    /must be borgmcp/,
  );
  await assert.rejects(
    () => createGithubRelease('3.14.0', githubReleaseBindingDeps({
      live: { name: 'borgmcp', version: '3.14.0', integrity: 'sha512-short' },
    })),

...
const SHARED_INTEGRITY = 'sha512-P04ATCkJJebqkMElnEKD1kGky6Cbmgs05wRsMAAWBzlPmzKVq2Yd2b3VP1BTxIJWOs5lfuJdzrBOdR5xxTW1Kw==';
    ...
    fetchImpl: async (url, options) => {
      ...
      return {
        ...
        json: async () => ({
          name: 'borgmcp-server',
          version: '1.0.1',
          dependencies: { 'borgmcp-shared': '1.0.1' },
        }),
      };
    },
  });
...
test('release preflight refuses a client before publication when the current server shared pin differs', async () => {
  await assert.rejects(
    ...
    /borgmcp@4\.2\.0 pins borgmcp-shared@1\.0\.1.*borgmcp-server@1\.0\.0 pins borgmcp-shared@0\.14\.0.*Publish the compatible server before tagging this client/s,
  );
});

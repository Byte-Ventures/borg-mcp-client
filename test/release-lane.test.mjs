...
const SHARED_INTEGRITY = 'sha512-sKCKCMBvJWHAVYtFAeBLfpuNZqSJgLqEy7/k1isefgJFlHTMJJSy53bOVj6W+pLWrRSKbhhFkxJheTmFey/cIg==';
    ...
    fetchImpl: async (url, options) => {
      ...
      return {
        ...
        json: async () => ({
          name: 'borgmcp-server',
          version: '4.0.2',
          dependencies: { 'borgmcp-shared': '2.2.0' },
        }),
      };
    },
  });
...
test('release preflight refuses a client before publication when the current server shared pin differs', async () => {
  await assert.rejects(
    ...
    /borgmcp@5\.4\.1 pins borgmcp-shared@2\.2\.0.*borgmcp-server@1\.0\.0 pins borgmcp-shared@0\.14\.0.*Publish the compatible server before tagging this client/s,
  );
});

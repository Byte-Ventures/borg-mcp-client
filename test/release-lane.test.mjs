...
const SHARED_INTEGRITY = 'sha512-wgx0iOK41bdngq6vqnzwaju+34uUOM3VM/ewrn4LfJlqP17CMjLq6xP7TOQ6E+itIfCh88u7zgGteu/j7HC0KQ==';
    ...
    fetchImpl: async (url, options) => {
      ...
      return {
        ...
        json: async () => ({
          name: 'borgmcp-server',
          version: '4.0.2',
          dependencies: { 'borgmcp-shared': '2.1.0' },
        }),
      };
    },
  });
...
test('release preflight refuses a client before publication when the current server shared pin differs', async () => {
  await assert.rejects(
    ...
    /borgmcp@5\.4\.0 pins borgmcp-shared@2\.1\.0.*borgmcp-server@1\.0\.0 pins borgmcp-shared@0\.14\.0.*Publish the compatible server before tagging this client/s,
  );
});

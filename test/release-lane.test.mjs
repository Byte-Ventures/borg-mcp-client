...
const SHARED_INTEGRITY = 'sha512-S8ynExOPIosDEeFl8TbeMNAaf+v+1RW2vk5IVlbP6M7rg1EKwWh4g17WVeozPmfF/n2qiyetCU5kPWz38cJ84w==';
...
test('release preflight refuses a client before publication when the current server shared pin differs', async () => {
  await assert.rejects(
    ...
    /borgmcp@5\.1\.2 pins borgmcp-shared@2\.0\.1.*borgmcp-server@1\.0\.0 pins borgmcp-shared@0\.14\.0.*Publish the compatible server before tagging this client/s,
  );
});

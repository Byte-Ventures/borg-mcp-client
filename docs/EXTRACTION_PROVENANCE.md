...
- Replaced the monorepo dependency on `borgmcp-shared` with the exact audited registry release `borgmcp-shared@0.4.0` and a fresh standalone lockfile.
...
resolves to
the audited registry `borgmcp-shared@0.4.0`; release remains blocked until the

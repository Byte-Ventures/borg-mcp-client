...
and pins the same `borgmcp-shared@0.7.0` contract. Client `borgmcp@2.7.0` is published. The immutable `v2.0.7`
attempt failed before publication and remains preserved. Publication of the next
candidate remains gated by reviewed `v2.7.1` source, a fresh annotated tag, and

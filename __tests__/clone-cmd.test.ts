...
function rig(base: string, quickstart = vi.fn(async () => 0)) {
  const output: string[] = [];
  const errors: string[] = [];
...
describe('clone credential boundary', () => {
  it.each([
    ...
    expect(await runClone({ repositoryUrl, noLaunch: true }, testRig.deps)).toBe(1);
  ...
  it('redacts credential-bearing Git diagnostics without rewriting safe argv', async () => {
    ...
    expect(await runClone({
      repositoryUrl: 'https://example.com/org/repo.git', destination: 'checkout', noLaunch: true,
    }, testRig.deps)).toBe(1);
...
describe('runClone', () => {
  it('fresh-clones and composes directly into quickstart in the checkout', async () => {
    ...
    const testRig = rig(base);
    expect(await runClone({ repositoryUrl: source, noLaunch: false }, testRig.deps)).toBe(0);
    const destination = join(base, 'checkouts', 'repo');
    expect(readFileSync(join(destination, 'README.md'), 'utf8')).toBe('ready\n');
    expect(testRig.quickstart).toHaveBeenCalledWith(destination);
  ...
  it('repeat-clone reuses only a matching origin', async () => {
    ...
    const source = sourceRepository(base);
    const testRig = rig(base);
    expect(await runClone({ repositoryUrl: source, noLaunch: true }, testRig.deps)).toBe(0);
    expect(await runClone({ repositoryUrl: source, noLaunch: true }, testRig.deps)).toBe(0);
  ...
  it('reports an explicit remote mismatch and leaves the checkout untouched', async () => {
    ...
    expect(await runClone({ repositoryUrl: source, destination, noLaunch: true }, testRig.deps)).toBe(0);
    expect(await runClone({ repositoryUrl: other, destination, noLaunch: true }, testRig.deps)).toBe(1);
  ...
  it('stops at a ready checkout with --no-launch', async () => {
    ...
    const source = sourceRepository(base);
    const testRig = rig(base);
    expect(await runClone({ repositoryUrl: source, destination: 'checkout', noLaunch: true }, testRig.deps)).toBe(0);
  ...
  it('keeps a successful checkout when quickstart fails and gives the recovery path', async () => {
    ...
    expect(await runClone({ repositoryUrl: source, noLaunch: false }, testRig.deps)).toBe(1);

...
describe('local server SSE adapter', () => {
  ...
  it.each([
  ...
  ] as const)('keeps a %s stream verdict terminal with zero retry', async (_case, terminalCode) => {
    ...
    const streamOnce = vi.fn(async () => { throw new BorgServerError(terminalCode, 'terminal session state'); });

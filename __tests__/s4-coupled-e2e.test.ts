...
const MAX_SNAPSHOT_KEYS = 10_000;
...
function snapshotPlainData(input: unknown): PlainSnapshot {
  const seen = new WeakSet<object>();
  let nodes = 0;
  let keysSeen = 0;
  const visit = (value: unknown, depth: number): unknown => {
    if (value === null || typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') return value;
    if (typeof value !== 'object' || depth > MAX_SNAPSHOT_DEPTH || ++nodes > MAX_SNAPSHOT_NODES || seen.has(value)) {
      throw new Error('invalid plain-data graph');
    }
    seen.add(value);
    ...
    if (Array.isArray(value)) {
      ...
      if (!lengthDescriptor || 'get' in lengthDescriptor || !Number.isSafeInteger(lengthDescriptor.value) || lengthDescriptor.value < 0 ||
        lengthDescriptor.value > MAX_SNAPSHOT_KEYS || keys.length !== lengthDescriptor.value + 1) throw new Error('invalid array shape');
      const result: unknown[] = [];
      for (let index = 0; index < lengthDescriptor.value; index += 1) {
        const descriptor = descriptors[String(index)];
        if (!descriptor || 'get' in descriptor || descriptor.enumerable !== true) throw new Error('sparse or accessor array');
    ...
    for (const key of keys as string[]) {
      ...
      if (!descriptor || 'get' in descriptor || descriptor.enumerable !== true) throw new Error('accessor or hidden property');
...
function validateS4CoupledSnapshot(value: unknown): value is S4CoupledE2EOutput {
...
export function validateS4CoupledE2EOutput(value: unknown): value is S4CoupledE2EOutput {
  ...
  return snapshot.ok && validateS4CoupledSnapshot(snapshot.value);
}

export const isS4CoupledResult = validateS4CoupledE2EOutput;

function isS4CoupledSnapshot(value: unknown): boolean {
  if (validateS4CoupledSnapshot(value)) return true;
...
export function isS4CoupledOutput(value: unknown): boolean {
  const snapshot = snapshotPlainData(value);
  return snapshot.ok && isS4CoupledSnapshot(snapshot.value);
}

function serializeS4Output(value: unknown, maxBytes = MAX_OUTPUT_BYTES): string | null {
  const snapshot = snapshotPlainData(value);
  if (!snapshot.ok || !isS4CoupledSnapshot(snapshot.value)) return null;
  try {
    const line = `S4_COUPLED_E2E ${JSON.stringify(snapshot.value)}`;
    return Buffer.byteLength(line, 'utf8') <= maxBytes ? line : null;
  } catch {
    return null;
  }
}
...
describe('Sprint 4 E2E harness validation', () => {
  ...
  it('rejects accessors without invoking them or emitting output', () => {
    const consoleLog = vi.spyOn(console, 'log').mockImplementation(() => {});
    let getterCalls = 0;
    const output = completeS4Output() as any;
    Object.defineProperty(output, 'pass', { enumerable: true, get: () => { getterCalls += 1; return getterCalls === 1 ? true : 'secret'; } });
    expect(() => isS4CoupledOutput(output)).not.toThrow();
    expect(isS4CoupledOutput(output)).toBe(false);
    expect(serializeS4Output(output)).toBeNull();
    expect(getterCalls).toBe(0);
    expect(consoleLog).not.toHaveBeenCalled();

    const nested = completeS4Output() as any;
    const socket: any = { event: 'socket_free' };
    Object.defineProperty(socket, 'socket_id', { enumerable: true, get: () => 'token=secret' });
    nested.phase.sockets = [socket];
    nested.phase.socket_event_count = 1;
    expect(isS4CoupledOutput(nested)).toBe(false);
    const failure = normalizedFailureOutput('https://127.0.0.1:7443', false, new Error('secret')) as any;
    Object.defineProperty(failure, 'error_code', { enumerable: true, get: () => FAILURE_CODE });
    expect(serializeS4Output(failure)).toBeNull();
  });

  it('captures proxy data descriptors once and serializes only the inert snapshot', () => {
    const target = completeS4Output();
    let getCalls = 0;
    let ownKeysCalls = 0;
    let passDescriptorCalls = 0;
    const proxy = new Proxy(target, {
      get(_target, property, receiver) {
        getCalls += 1;
        return property === 'pass' && getCalls > 1 ? 'token=secret' : Reflect.get(_target, property, receiver);
      },
      ownKeys(_target) {
        ownKeysCalls += 1;
      ...
      getOwnPropertyDescriptor(_target, property) {
        const descriptor = Reflect.getOwnPropertyDescriptor(_target, property);
        if (property === 'pass') passDescriptorCalls += 1;
        return descriptor;
      },
    });
    const serialized = serializeS4Output(proxy);
    expect(serialized).toBe(serializeS4Output(target));
    expect(serialized).not.toMatch(/token|secret/);
    expect(getCalls).toBe(0);
    expect(ownKeysCalls).toBe(1);
    expect(passDescriptorCalls).toBe(1);
  });

  it.each(['getPrototypeOf', 'ownKeys', 'getOwnPropertyDescriptor'] as const)(
    'rejects a proxy with a throwing %s trap without throwing or emitting',
    (trap) => {
      const consoleLog = vi.spyOn(console, 'log').mockImplementation(() => {});
      const proxy = new Proxy(completeS4Output(), { [trap]: () => { throw new Error('/private/ca.pem token=secret'); } });
      expect(() => isS4CoupledOutput(proxy)).not.toThrow();
      expect(isS4CoupledOutput(proxy)).toBe(false);
      expect(serializeS4Output(proxy)).toBeNull();
      expect(consoleLog).not.toHaveBeenCalled();
    },
  );

  it('rejects sparse arrays, symbols, custom prototypes, and cycles at the snapshot boundary', () => {
    const sparse = completeS4Output() as any;
    sparse.phase.sockets = new Array(1);
    sparse.phase.socket_event_count = 1;
    expect(isS4CoupledOutput(sparse)).toBe(false);
    const symbol = completeS4Output() as any;
    symbol[Symbol('secret')] = 'token';
    expect(isS4CoupledOutput(symbol)).toBe(false);
    const custom = completeS4Output() as any;
    Object.setPrototypeOf(custom.phase, { inherited: true });
    expect(isS4CoupledOutput(custom)).toBe(false);
    const cyclic = completeS4Output() as any;
  ...
  it('enforces the persistent runner line byte limit using UTF-8 bytes', () => {
    const serialized = serializeS4Output(completeS4Output());
    ...
    expect(serializeS4Output(completeS4Output(), lineBytes - 1)).toBeNull();
    ...
    expect(serializeS4Output(oversized)).toBeNull();
  });
...
describe.runIf(enabled)('Sprint 4 joined client/server E2E', () => {
  it('proves idle=0, directed=1, exact 150 drain, transport health, and zero egress', async () => {
    ...
    if (operationError) {
      ...
      const failureLine = serializeS4Output(failureOutput);
    ...
    const successLine = serializeS4Output(output);
    if (!successLine) throw new Error('final S4 coupled E2E output serialization rejected');

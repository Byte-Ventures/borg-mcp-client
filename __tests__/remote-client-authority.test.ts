...
describe('remote-client explicit authority connection', () => {
  ...
  it.each([
    ['SESSION_REJECTED', JSON.stringify({ protocol_version: '8', error: { code: 'SESSION_REJECTED', message: 'no' } }), 'SESSION_REJECTED'],
    ['SESSION_REVOKED', JSON.stringify({ protocol_version: '8', error: { code: 'SESSION_REVOKED', message: 'no' } }), 'SESSION_REVOKED'],
  ...
  it.each([
    ['exact code on a parent request', JSON.stringify({ protocol_version: '8', request_id: 'parent-1', error: { code: 'DRONE_EVICTED', message: 'no' } })],
    ['wrong code', JSON.stringify({ protocol_version: '8', request_id: 'wrong-1', error: { code: 'ACCESS_DENIED', message: 'no' } })],
    ['bare body', 'gone'],
    ['malformed body', '{'],

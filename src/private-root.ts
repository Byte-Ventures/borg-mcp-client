...
function sameIdentity(
  left: { dev: number | bigint; ino: number | bigint; uid: number | bigint; mode: number | bigint },
  right: { dev: number | bigint; ino: number | bigint; uid: number | bigint; mode: number | bigint },
): boolean {
  return left.dev === right.dev && left.ino === right.ino && left.uid === right.uid &&
    (Number(left.mode) & 0o777) === (Number(right.mode) & 0o777);
}

async function inspectRoot(root: string): Promise<Awaited<ReturnType<typeof lstat>>> {
...
export async function ensurePrivateBorgConfigRoot(): Promise<PrivateBorgConfigRoot> {
  ...
  try {
    ...
    if (!sameIdentity(before, opened)) throw new Error(`Borg config root ${root} changed during verification`);

 ...
 * "Authentication".
 */
export function authRecoveryMessage(error: AuthErrorLike): string | null {
  const message = error.message ?? '';
  if (error.name === 'RefreshTransientError' || message.includes('Failed to refresh')) {
    return (
      '◼ Transient auth-refresh failure (network/Google hiccup). Your session is intact ' +
  ...
  if (message.includes('Authentication required') || message.includes('Authentication expired')) {
    return (
      '◼ Authentication expired — re-consent needed. Run `borg setup` in a terminal to sign in again. ' +
      'Do NOT re-assimilate: borg_assimilate cannot fix auth and would mint a duplicate drone seat.'
    );
  }
  return null;

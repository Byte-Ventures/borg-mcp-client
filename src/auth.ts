...
export class RefreshTokenInvalidError extends Error {
  constructor(public readonly errorCode: string, public readonly errorDescription?: string) {
    super(
      errorDescription
...
function sendCallbackHtml(res: ServerResponse, status: number, body: string): void {
...
export async function startCallbackServer(expectedState: string): Promise<{
  ...
  timeout = setTimeout(() => {
    ...
    rejectCode(new Error(
      `Authentication timed out after ${AUTH_CALLBACK_TIMEOUT_MIN} minutes — ` +
 ...
 */
async function revokeRefreshTokenAtGoogle(refreshToken: string): Promise<void> {
  try {
 ...
 */
async function authenticateWithBrowser(): Promise<void> {
  ...
  try {
    ...
    cerr(
      `Could not open a browser automatically: ${(err as Error)?.message ?? 'unknown'}`
    );
  ...
  cerr(
    `Waiting for you to finish signing in (up to ${AUTH_CALLBACK_TIMEOUT_MIN} ` +
 ...
 */
export function shouldUseDeviceFlow(opts?: { noBrowser?: boolean }): boolean {
 ...
 */
export function buildDeviceAuthConfig(env: NodeJS.ProcessEnv = process.env): DeviceAuthConfig {
  ...
  if (!clientId) {
    throw new Error(
      'No-browser (device-grant) auth needs a Google "TVs & Limited Input devices" ' +
...
export async function authenticateWithDeviceFlow(
  deps: DeviceAuthDeps = { fetch, sleep: defaultSleep },
 ...
 */
export async function authenticateWithGoogle(opts?: { noBrowser?: boolean }): Promise<void> {
...
/**
 * Outcome of a single per-client refresh attempt — returned, not thrown,
 * so `refreshIdToken`'s web-then-device cascade can weigh both attempts
 * before deciding whether the token is genuinely revoked (gh#691).
 */
type RefreshAttemptResult =
  | { ok: true }
  | { ok: false; kind: 'invalid'; error: RefreshTokenInvalidError }
  // gh#858: `persistFailed` marks a transient that occurred AFTER a successful
  // Google exchange (the local credential-store WRITE failed). The exchange
  // worked, so re-running it against the other client is pointless — the
  // orchestrator short-circuits on this flag.
  | { ok: false; kind: 'transient'; error: RefreshTransientError; persistFailed?: boolean };
 ...
 */
async function attemptRefreshWithClient(
  refreshToken: string,
  clientId: string,
  ...
  try {
    ...
    return {
      ok: false,
      kind: 'transient',
      error: new RefreshTransientError(
        `Network failure during token refresh: ${(err as Error)?.message ?? 'unknown'}`
      ),
    };
  }
  ...
  if (!response.ok) {
    ...
    let errBody: { error?: string; error_description?: string } | null = null;
    try {
      errBody = (await response.json()) as { error?: string; error_description?: string };
    } catch {
      return {
        ok: false,
        kind: 'transient',
        error: new RefreshTransientError(
          `Token refresh failed with HTTP ${response.status} (non-JSON body)`
        ),
      };
    }
    ...
    return {
      ...
      kind: 'transient',
      error: new RefreshTransientError(
        `Token refresh failed with HTTP ${response.status}${errBody?.error ? ` (${errBody.error})` : ''}`
      ),
    };
  }
  ...
  try {
    ...
    return {
      ...
      kind: 'transient',
      error: new RefreshTransientError(
        `Token refresh response unparseable: ${(err as Error)?.message ?? 'unknown'}`
      ),
    };
  }
  ...
  if (!data.id_token || typeof data.expires_in !== 'number') {
    return {
      ...
      kind: 'transient',
      error: new RefreshTransientError(
        'Token refresh response missing id_token or expires_in'
      ),
    };
  }
  ...
  try {
    ...
    return {
      ...
      persistFailed: true,
      error: new RefreshTransientError(
        `Token refresh succeeded but saving it to the credential store failed ` +
 ...
 */
async function migrateToFileBackendAndPersist(
  data: { id_token?: string; refresh_token?: string },
  expiresAt: number
): Promise<boolean> {
  if (!data.id_token) return false; // nothing to persist (TS narrowing is reset in catch)
  let migrated = false;
  try {
    if (!(await isUsingKeychainBackend())) return false;
    ...
    if (!effectiveRefresh) return false;
 ...
 */
function warnFileFallbackTradeoff(): void {
 ...
 */
export async function refreshIdToken(refreshToken: string): Promise<void> {
  const web = await attemptRefreshWithClient(
    refreshToken,
    GOOGLE_CLIENT_ID,
  ...
  if (web.kind === 'transient' && web.persistFailed) throw web.error;

  const device = buildDeviceAuthConfig();
  const dev = await attemptRefreshWithClient(
    refreshToken,
    device.clientId,
    device.clientSecret
  );

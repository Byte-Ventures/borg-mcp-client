...
export const TOOL_MANIFEST: ToolManifestEntry[] = [
        ...
        },
        {
          name: 'borg_docs',
          description:
            'Look up the Borg MCP documentation. Call this when the user asks how borgmcp works, or any feature / usage / setup / pricing / concept / tool question. Returns the docs index — each section\'s borgmcp.ai URL + a one-line summary. Pass `topic` (e.g. "pricing", "worktree", "roles", "codex") to get the best-matching section(s) instead of the full index. Then WebFetch the returned URL to read the page — borg_docs returns the index only, it does not fetch the page for you.',
          inputSchema: {
            type: 'object',
            properties: {
              topic: {
        ...
        },
        {
          ...
          description:
            'Remove one active ratified decision from the cube registry by topic or decision id. SEAT-HOLDER ONLY (Coordinator/Queen). The decision stops appearing in borg_decisions and borg_regen while its audit record is retained.',
          inputSchema: {
            type: 'object',
          ...
          inputSchema: {
            ...
            properties: {
              message: {
                type: 'string',
                description:
                  'What hit you + what you expected instead (max 10KB). Concrete and specific helps the dev team most.',
              },
              kind: {
                type: 'string',
                enum: ['friction', 'bug'],
              ...
              metadata: {
                type: 'object',
                description:
                  'Optional non-secret context. Allowed keys only: version, cube_id, os. Any other key is rejected.',
              },
            },
            required: ['message'],
          },
        ...
        },
        {
          ...
          description:
            'Reassign a drone to a different role in the same cube. Coordinator-shaped: the cube\'s Coordinator drone is the one expected to call this when dispatching new drones to specific work. ' +
        ...
        },
        {
          ...
          description:
            'List every drone in a cube (owner-scoped). Returns id, label, role_id, agent_kind, last_seen, advisory reported model, working repository, and wake_path_alert_class for each — gives the Coordinator a roster they can act on with borg_reassign-drone.',
          inputSchema: {
            type: 'object',
            properties: {

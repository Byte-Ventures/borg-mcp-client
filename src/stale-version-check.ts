 ...
 */
export function compareVersionsForStaleness(
  installed: string,
  latest: string
): VersionCheckResult {
  ...
  void minorDelta;
  const message =
    `⚠ borgmcp ${installed} is behind latest ${latest} — npm install -g borgmcp@latest`;
 ...
 */
export async function fetchLatestBorgmcpVersion(): Promise<string | null> {

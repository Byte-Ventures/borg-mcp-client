 ...
 * decision funnel.
 */
export function isNoBrowserEnv(probe?: Partial<BrowserEnvProbe>): boolean {
  const env = probe?.env ?? process.env;
  const platform = probe?.platform ?? process.platform;

  // 1. Explicit force-browser escape hatch wins over every no-browser signal.
  if (envToggleOn(env.BORG_FORCE_BROWSER)) return false;

  // 2. Explicit opt-in to the no-browser/device flow.
  if (envToggleOn(env.BORG_NO_BROWSER)) return true;

...
export async function prepareConnection(
  args: AssimilateArgs,
  deps: AssimilateDeps,
  options: { validateRole: (role: Role) => void; onPrepared: (prepared: PreparedAssimilation) => void },
): Promise<number> {
...
async function runAssimilationFlow(
  ...
  if (repositoryContext.publicRepository && deps.hasActiveSeatInDifferentCloneFamily) {
    try {
      if (await deps.hasActiveSeatInDifferentCloneFamily(
        result.cube_id,
        repositoryContext.publicRepository.value,
        repositoryContext.commonDir,
      )) {
        deps.stderr(

...
export async function requestDeviceCode(
  ...
  try {
    ...
    throw new DeviceAuthError(
      ...
      `Could not reach Google device endpoint: ${(err as Error)?.message ?? 'unknown'}`
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new DeviceAuthError(
      ...
      `Device-code request failed (HTTP ${response.status}${code ? `, ${code}` : ''})`
    );
  }

  let data: Partial<DeviceCodeResponse>;
  try {
    data = (await response.json()) as Partial<DeviceCodeResponse>;
  } catch {
  ...
  if (!data.device_code || !data.user_code || !data.verification_url) {
    throw new DeviceAuthError(
      ...
      'Device-code response missing device_code/user_code/verification_url'
    );
  }
...
export async function pollForDeviceToken(
  ...
  while (true) {
    if (now() >= deadline) {
      throw new DeviceAuthError(
        'expired_token',
        'Device code expired before the authorization was completed'
      );
    }
    ...
    try {
      ...
      throw new DeviceAuthError(
        ...
        `Could not reach Google token endpoint: ${(err as Error)?.message ?? 'unknown'}`
      );
    }

    if (response.ok) {
      let data: Partial<DeviceTokenResult>;
      try {
        data = (await response.json()) as Partial<DeviceTokenResult>;
      } catch {
      ...
      if (!data.id_token || typeof data.expires_in !== 'number') {
        throw new DeviceAuthError(
          'malformed_token_response',
          'Token response missing id_token or expires_in'
        );
      }
      return {
    ...
    switch (code) {
      ...
      default:
        throw new DeviceAuthError(
          code ?? 'device_token_request_failed',
          `Device token poll failed (HTTP ${response.status}${code ? `, ${code}` : ''})`
        );
    }
  }
}

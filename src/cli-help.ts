...
export function recoverEnrollmentHelpText(version: string): string {
  return (
    `borg recover-enrollment (borgmcp ${version}) — restore or clear one failed server enrollment\n\n` +
    `Usage:\n` +
...
export function cloneHelpText(version: string): string {
  return (
    `borg clone (borgmcp ${version}) — clone a repository and prepare a sibling worktree\n\n` +
    `Usage:\n` +

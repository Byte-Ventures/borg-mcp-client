 ...
 */
export function composeGetStarted(hasAgentCli: boolean): string {
  const lines = [
    '',
  ...
  if (!hasAgentCli) {
    lines.push(
      `  ${step}. Install an agent CLI first:`,
  ...
  step++;
  lines.push(
    `  ${step}. cd into your project, then: borg assimilate — join/create a cube`,
    '',
    'Then `borg` launches your agent in that cube. Run `borg --help` for more.',
    '',
  );

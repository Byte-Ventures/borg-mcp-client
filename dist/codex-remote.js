 ...
 */
function sanitizeCodexDiagnostic(input) {
    ...
    sanitized = sanitized.replace(/([?&](?:access_token|auth_token|id_token|refresh_token|session_token|token|api_key|apikey|secret|password)=)[^&#\s]*/gi, '$1<REDACTED>');
    sanitized = sanitized.replace(/\b((?:[a-z0-9_-]*(?:token|secret|password)|api[_-]?key)\s*[:=]\s*)(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\s,;]+)/gi, '$1<REDACTED>');
    sanitized = sanitized.replace(/\beyJ[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{6,}\b/g, '<REDACTED>');
    sanitized = sanitized.replace(/(^|[\s("'=])(?:file:\/\/)?\/(?:Users|home|private|tmp|var|etc|opt|Volumes)\/[^\s"'`<>)}\]]+/gm, '$1<REDACTED_PATH>');
    sanitized = sanitized.replace(/\b[A-Za-z]:\\(?:Users|Documents and Settings|ProgramData|Temp)\\[^\s"'`<>)}\]]+/g, '<REDACTED_PATH>');
...
function refuse(reason, stderr = '') {
    return { ready: false, reason, stderr: sanitizeCodexDiagnostic(stderr).trim() };
}
function formatExitReason(diagnostics) {
    if (diagnostics.error)
        return `process error: ${sanitizeCodexDiagnostic(diagnostics.error)}`;
    if (diagnostics.exitCode != null)
        return `exit code ${diagnostics.exitCode}`;
    if (diagnostics.signal)
        return `signal ${diagnostics.signal}`;
...
export async function prepareCodexRemoteLaunch(deps) {
    ...
    for (let launchAttempt = 0; launchAttempt < 2; launchAttempt += 1) {
        ...
        try {
            ...
            safeRm(socketPath);
            safeRm(pidPath);
            lastRefusal = refuse(`could not start \`codex app-server\` (${sanitizeCodexDiagnostic(String(err?.message ?? err))}); confirm the Codex executable is installed and available on PATH`);
            continue;
        }
        if (child.pid != null) {
            try {
        ...
        if (!ready) {
            cleanup();
            if (diagnostics?.exited) {
                lastRefusal = refuse(`\`codex app-server\` exited before becoming ready (${formatExitReason(diagnostics)})`, diagnostics.stderr);
            }
            else {
                lastRefusal = refuse(`\`codex app-server\` remained running but did not become ready within ${readyTimeoutMs}ms`, diagnostics?.stderr ?? '');
            }
            continue;
        }
        return {

...
function resolveCommit(ref, cwd, runGit) {
    let result;
    try {
...
export function validateRefs(value) {
    if (value === undefined)
        return [];
    ...
    for (const valueRef of value) {
        ...
        if (valueRef.startsWith('-'))
            throw new Error(`Git ref must not start with "-": ${valueRef}`);
        ...
        if (refs.includes(valueRef))
            throw new Error(`Git ref is duplicated: ${valueRef}`);
        refs.push(valueRef);
    }
    return refs;
}
export function resolveRefs(refs, cwd, runGit = defaultRunGit) {
    return refs.map((ref) => {
...
export function auditMessageShas(message, cwd, runGit = defaultRunGit) {
    const shas = [...new Set(message.match(MESSAGE_SHA_RE) ?? [])];
    const unverified = [];
    for (const sha of shas) {
        if (resolveCommit(sha, cwd, runGit).sha)
            continue;
...
export function renderProvenance(resolved) {
    return resolved.length === 0
        ? ''
        : `\n\n${resolved.map(({ ref, sha }) => `${ref} = ${sha}`).join('\n')}`;
}
export function requiresRefs(message) {
    return /^REVIEW-READY(?:\b|$)/u.test(message);
}
//# sourceMappingURL=log-provenance.js.map
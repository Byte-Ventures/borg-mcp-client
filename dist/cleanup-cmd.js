...
export function isRegenerableIgnored(ignoredPath) {
    const clean = ignoredPath.replace(/\/+$/, ''); // strip trailing slash
    const segments = clean.split('/').filter((s) => s.length > 0);
    if (segments.some((s) => REGENERABLE_DIRS.has(s)))
        return true;
    const base = segments[segments.length - 1] ?? clean;
    if (REGENERABLE_FILES.has(base))
        return true;
    if (REGENERABLE_SUFFIXES.some((suf) => base.endsWith(suf)))
        return true;
    return false;
}
/**
 ...
 */
export function clobberClassIgnored(runSync, cwd) {
 ...
 */
export function isStrictlyUnder(realpath, parent, child) {
    let rp;
    let rc;
    try {
        rp = realpath(parent);
    ...
    if (rc === rp)
        return false; // the home dir itself is not "under" itself
 ...
 */
export function parseWorktreeEntries(porcelain) {
    const entries = [];
    let cur = null;
 ...
 */
async function classifyWorktree(deps, worktreePath, actualBranch, seat) {
    const { runSync } = deps;
    // (1) dirty — tracked staged/unstaged/untracked. KEEP.
    const dirty = classifyDirty(runSync, worktreePath);
    const dirtyCount = dirty.staged.length + dirty.unstaged.length + dirty.untracked.length;
    if (dirtyCount > 0) {
 ...
 */
export async function buildCleanupReport(deps) {
    ...
    const seatByReal = new Map();
    for (const { projectPath, cube } of seats) {
        const real = safeRealpath(realpath, projectPath);
        if (real)
            seatByReal.set(real, cube);
    }
    const rows = [];
    for (const wt of allWorktrees) {
        const real = safeRealpath(realpath, wt);
        if (!real)
            continue; // gone from disk — git will report it as prunable itself
        if (mainReal && real === mainReal)
            continue; // primary worktree — never a candidate
 ...
 */
export async function runCleanup(deps = {}, opts = { prune: false }) {
    ...
    for (const r of prunable) {
        ...
        let baseNote = '';
        const derivedWtBranch = perWorktreeBranchName(basename(r.worktreePath), basename(dirname(r.worktreePath)));
        if (derivedWtBranch !== r.wtBranch &&
...
export function parseCleanupArgs(rawArgs) {
    let prune = false;
...
//# sourceMappingURL=cleanup-cmd.js.map
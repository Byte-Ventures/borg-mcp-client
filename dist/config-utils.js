...
export function addOpenCodeMcpServer() {
    try {
        ...
        const stateRootEnvArg = stateRoot
            ? ` --env ${BORG_STATE_ROOT_ENV}=${shellQuote(stateRoot)}`
            : '';
        execSync(`opencode mcp add borg --env BORG_SESSION=1 --env BORG_AGENT_KIND=opencode --env BORG_OPENCODE=1${apiUrlEnvArg}${stateRootEnvArg} -- ${shellQuote(MCP_COMMAND)}`, { stdio: 'inherit', env: borgAgentConfigEnv(process.env) });
    }
    catch (error) {
        if (error.message?.includes('command not found')) {

...
import { resolveRegenPath, resolveClearRewakePath, resolveLogAuditPath, resolveForeignPathReminderPath, } from './self-path.js';
...
import { OPENCODE_SERVER_PASSWORD_ENV, OPENCODE_SERVER_PASSWORD_REFERENCE, } from './opencode-launch-trust.js';
...
const OPENCODE_GLOBAL_CONFIG_FILENAMES = ['config.json', 'opencode.json', 'opencode.jsonc'];
...
function mergeOpenCodeConfigValue(target, source) {
    if (!isPlainRecord(target) || !isPlainRecord(source))
        return source;
    const merged = Object.assign(Object.create(null), target);
    for (const [key, value] of Object.entries(source)) {
        merged[key] = key in merged ? mergeOpenCodeConfigValue(merged[key], value) : value;
    }
    return merged;
}
function readOpenCodeGlobalConfig(configPath) {
    const basename = path.basename(configPath);
    const paths = OPENCODE_GLOBAL_CONFIG_FILENAMES.includes(basename)
    ...
    for (const candidate of paths) {
        ...
        if (!isPlainRecord(parsed))
            throw new TypeError(`OpenCode config is not an object: ${candidate}`);
        merged = mergeOpenCodeConfigValue(merged, parsed);
    }
    if (!isPlainRecord(merged))
        throw new TypeError('OpenCode global config is not an object');
    return merged;
}
...
function unsafeOpenCodeConfigPath(configPath, detail) {
    return new Error(`OpenCode config path ${configPath} is unsafe: ${detail}`);
}
function assertOpenCodeProjectConfigDirectory(projectRoot, configPath) {
...
function inspectOpenCodeProjectConfigFile(configPath) {
    let metadata;
    try {
...
function writeOpenCodeProjectConfig(projectRoot, configPath, data, snapshot) {
    const configDir = assertOpenCodeProjectConfigDirectory(projectRoot, configPath);
    const temporary = path.join(configDir, `.opencode.json.${process.pid}.${randomBytes(12).toString('hex')}.tmp`);
    let descriptor = null;
    try {
        descriptor = fs.openSync(temporary, fs.constants.O_WRONLY
            | fs.constants.O_CREAT
            | fs.constants.O_EXCL
            | (fs.constants.O_NOFOLLOW ?? 0), snapshot.mode);
 ...
 */
export function addClaudeLaunchAccess(projectRoot, paths) {
    ...
    const existingDirectories = Array.isArray(additionalDirectories)
...
function ownedCanonical(command) {
    ...
    if (stripped === BARE_FOREIGN_PATH_REMINDER)
        return FOREIGN_PATH_REMINDER_HOOK_COMMAND;
    ...
    if (command === AUDIT_HOOK_COMMAND || stripped === resolveLogAuditPath())
        return AUDIT_HOOK_COMMAND;
    if (command === FOREIGN_PATH_REMINDER_HOOK_COMMAND || stripped === resolveForeignPathReminderPath())
        return FOREIGN_PATH_REMINDER_HOOK_COMMAND;
    ...
    if (stripped.startsWith('/') && packageShaped) {
        ...
        if (name === 'log-audit.js')
            return AUDIT_HOOK_COMMAND;
        if (name === 'foreign-path-reminder.js')
            return FOREIGN_PATH_REMINDER_HOOK_COMMAND;
    }
    return null;
}
...
function migrateAndDedupOwnedHooks(entries) {
    ...
    const emptiedByOwnedDedup = new Set();
    for (const entry of entries) {
        if (!Array.isArray(entry?.hooks))
            continue;
        ...
        if (entry.hooks.length !== before)
            changed = true;
        if (before > 0 && entry.hooks.length === 0)
            emptiedByOwnedDedup.add(entry);
    }
...
function globalManagedAgentHookConfigPaths(homeDir) {
    return [
        path.join(homeDir, '.claude', 'settings.json'),
        path.join(homeDir, '.codex', 'hooks.json'),
    ];
}
function realDirectory(pathname) {
    try {
...
export function managedAgentHookConfigPaths(homeDir = CONFIG_HOME) {
    ...
    for (const repo of fs.readdirSync(root, { withFileTypes: true })) {
        ...
        if (!repo.isDirectory() || !realDirectory(repoPath))
            continue;
        for (const worktree of fs.readdirSync(repoPath, { withFileTypes: true })) {
            const worktreePath = path.join(repoPath, worktree.name);
            if (!worktree.isDirectory() || !realDirectory(worktreePath))
                continue;
            const claudeDir = path.join(worktreePath, '.claude');
            if (!realDirectory(claudeDir))
                continue;
            const settingsFile = path.join(claudeDir, 'settings.local.json');
            try {
                if (fs.lstatSync(settingsFile).isSymbolicLink())
                    continue;
            }
            catch {
...
function refreshHookFile(configPath) {
 ...
 */
export function refreshManagedAgentHookConfigs(options = {}) {
    ...
    for (const configPath of configPaths) {
        try {
            if (refreshHookFile(configPath))
                refreshed.push(configPath);
        }
        catch (error) {
...
export function inspectManagedAgentHookConfigs(homeDir = CONFIG_HOME) {
    let configPaths = globalManagedAgentHookConfigPaths(homeDir);
    let inventoryIssue = null;
    try {
        configPaths = managedAgentHookConfigPaths(homeDir);
    }
    catch (error) {
    ...
    const health = configPaths.map((configPath) => {
        if (!fs.existsSync(configPath))
            return { path: configPath, status: 'absent' };
        try {
            const config = readJsonFile(configPath);
            if (!config || typeof config !== 'object' || Array.isArray(config)) {
            ...
            if (config.hooks === undefined)
                return { path: configPath, status: 'ok' };
            ...
            for (const entries of Object.values(copy)) {
                if (Array.isArray(entries))
                    stale = migrateAndDedupOwnedHooks(entries) || stale;
            }
...
function configuredMcpCommand(value) {
...
function isStaleBorgMcpCommand(value) {
    const command = configuredMcpCommand(value);
    if (!command || command === MCP_COMMAND || !path.isAbsolute(command))
        return false;
    const normalized = command.replace(/\\/g, '/');
    return /\/(?:node_modules\/borgmcp|borg-mcp-client)\/dist\/index\.js$/.test(normalized);
}
function readClaudeMcpCommand(configPath) {
    try {
...
function readCodexMcpCommand(configPath) {
    try {
        const text = fs.readFileSync(configPath, 'utf-8');
        if (!isCodexMcpServerConfigured(configPath))
            return undefined;
...
function readOpenCodeMcpCommand(configPath) {
    try {
        const borgServer = readJsonFile(configPath)?.mcp?.[MCP_SERVER_NAME];
        if (!borgServer || borgServer.type !== 'local')
            return undefined;
...
function addCodexHook(eventName, command, options = {}, hooksPath = CODEX_HOOKS_PATH) {
    let hooksFile;
    try {
    ...
    if (changed)
        writeJsonFile(hooksPath, hooksFile);
    return changed;
}
...
export function addCodexForeignPathReminderHook(hooksPath = CODEX_HOOKS_PATH) {
...
export function isOpenCodeMcpServerConfigured(configPath = path.join(borgHomeRoot(), '.config', 'opencode', 'opencode.json')) {
    try {
        const borgServer = readOpenCodeGlobalConfig(configPath).mcp?.borg;
 ...
 */
export function isOpenCodeMcpServerConfiguredForLaunch(configPath = path.join(borgHomeRoot(), '.config', 'opencode', 'opencode.json'), env = process.env) {
    if (!isOpenCodeMcpServerConfigured(configPath))
        return false;
    try {
        const borgServer = readOpenCodeGlobalConfig(configPath).mcp?.borg;
        const environment = borgServer?.environment ?? borgServer?.env;
        ...
        if (!env[BORG_LAUNCH_EXPECTED_SEAT_ENV])
            return true;
 ...
 */
export function addOpenCodeLaunchAccess(projectRoot, paths) {
...
export function addOpenCodeMcpServer() {
    try {
        ...
        const stateRootEnvArg = stateRoot
            ? ` --env ${BORG_STATE_ROOT_ENV}=${shellQuote(stateRoot)}`
            : '';
        const launchExpectedSeatEnvArg = ` --env ${BORG_LAUNCH_EXPECTED_SEAT_ENV}=${shellQuote(OPENCODE_LAUNCH_EXPECTED_SEAT_REFERENCE)}`;
        const apiPasswordEnvArg = ` --env ${OPENCODE_SERVER_PASSWORD_ENV}=${shellQuote(OPENCODE_SERVER_PASSWORD_REFERENCE)}`;
        execSync(`opencode mcp add borg --env BORG_SESSION=1 --env BORG_AGENT_KIND=opencode --env BORG_OPENCODE=1${apiUrlEnvArg}${stateRootEnvArg}${launchExpectedSeatEnvArg}${apiPasswordEnvArg} -- ${shellQuote(MCP_COMMAND)}`, { stdio: 'inherit', env: borgAgentConfigEnv(process.env) });
    }
    catch (error) {
        if (error.message?.includes('command not found')) {

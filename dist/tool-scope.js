...
export const DISPATCHER_TOOLS = ['borg_tool', 'borg_describe-tool'];
...
export function isManagementSeat(scope) {
 ...
 * set stays native.
 */
export function deferredToolNames(scope) {
    if (!scope || !scope.roleName)
        return new Set(); // role unknown → full set
    if (isManagementSeat(scope))
        return new Set(); // management seat → full set
    return new Set([...MANAGEMENT_TOOLS, ...BILLING_TOOLS]); // worker → defer
}
/**
 ...
 * via the dispatcher.
 */
export function filterToolsForRole(tools, scope) {

...
let _moduleSettleOpenCodeEntry;
export function setModuleInjectOpenCode(fn, settle) {
        ...
        while (_iterations < _maxIterations) {
            ...
            try {
                ...
                if (err instanceof CubeDeletedError || err instanceof DroneEvictedError || err instanceof BorgServerError) {
                    ...
                    const replacementChanged = replacement !== null && (replacement.localSessionCredentialRef !== active.localSessionCredentialRef ||
...
export async function streamOnce(active, lastEventId, onEventId, deps = {}) {
    ...
    let lastPersistedEventId = cursor?.id ?? lastEventId;
    ...
    const shouldDeliverWakeRetry = async (entryId) => {
        try {
            if (await hasPendingWakeEntry(active, entryId))
                return true;
    ...
    const writeInboxLine = async (ev) => {
        ...
        const eventHwm = ev.data?.created_at
            ...
            : null;
        const atOrBeforeResumeCursor = Boolean(isCatchingUp &&
            lastPersistedHwm &&
            eventHwm &&
        ...
        const alreadyPersisted = atOrBeforeResumeCursor || (consultInboxFile &&

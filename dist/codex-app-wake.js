...
import { codexAppServerSocketFromEnv, pickFreshThread, wakeRetryBackoffMs, wakeRetryExpired, WAKE_RETRY_MAX_ATTEMPTS, shouldFireHeartbeat, } from './codex-wake-resolve.js';
 ...
 */
export async function probeCodexBridgeArmed(_active, deps = {}) {
    try {
        const socketPath = codexAppServerSocketFromEnv(deps.env ?? process.env);
        if (!socketPath)
            return false;
        if (!(deps.socketExists ?? existsSync)(socketPath))
            return false;
...
function injectionFailureCode(err) {
    const code = err?.code;
...
function recordInjectionResult(result, now, failureCode) {
    lastInjectionAt = now();
...
async function resolveFreshCodexWakeTarget(_active, deps) {
    ...
    try {
        ...
        for (const id of ids) {
            ...
            if (t)
                summaries.push({ id: t.id, cwd: t.cwd, updatedAt: t.updatedAt, source: t.source });
        }

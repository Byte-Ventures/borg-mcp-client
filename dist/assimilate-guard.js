...
export function reattachOnlyRefusal(decision, requestedCubeName) {
    if (decision.kind === 'no-identity') {
        return (`◼ This session has no drone for this worktree, and in-session borg_assimilate is ` +
...
export function reattachFailureMessage(error) {
    ...
    return (`◼ Re-attach failed — this worktree's saved connection is unreachable (likely evicted or its ` +

 ...
 */
export function matchesOnlyLabel(droneLabel, only) {
 ...
 */
export function enumerateLinkedWorktrees(runSync) {
    let raw;
    try {
        raw = runSync('git', ['worktree', 'list', '--porcelain']);
    }
    catch (e) {
        throw new Error(`launch-all: git worktree list failed — must be run from inside a git repository\n` +
 ...
 */
export async function discoverDroneCandidates(opts, deps) {
    ...
    for (const worktreeDir of worktreePaths) {
        ...
        if (!deps.pathExists(worktreeDir)) {
            deps.stderr(`skipping ${worktreeDir}: directory not found (orphaned worktree — run \`git worktree prune\`)\n`);
            continue;
        }
        ...
        if (cube.cubeId !== opts.targetCubeId)
            continue;
        // 4. malformed entry (cubeId/droneId not UUID) → warn + skip
        if (!isUuid(cube.cubeId) || !isUuid(cube.droneId)) {
            deps.stderr(`skipping ${worktreeDir}: its saved connection has a malformed cubeId/droneId — re-assimilate to fix\n`);
            continue;
        }
        // 5. --only filter (tier-1 label) → skip silently (counted in the filter report)
        if (opts.only !== undefined && !matchesOnlyLabel(cube.droneLabel, opts.only))
            continue;
        candidates.push({
            worktreeDir,
            cubeId: cube.cubeId,
...
//# sourceMappingURL=launch-all-discovery.js.map
...
async function launchMacOS(candidates, opts, deps) {
    ...
    if (!hasITerm && !hasTerminal) {
        throw new NoTerminalError('borg launch-all: --mode windows requires a compatible terminal app.\n' +
...
async function launchLinux(candidates, opts, deps) {
    ...
    if (!term) {
        throw new NoTerminalError('borg launch-all: --mode windows requires a terminal emulator.\n' +
...
export async function runWindowsBackend(candidates, opts, deps) {
...
//# sourceMappingURL=launch-all-windows.js.map
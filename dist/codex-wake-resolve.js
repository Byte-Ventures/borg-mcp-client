...
export function codexAppServerSocketFromEnv(env = process.env) {
 ...
 */
export function codexAppServerSocketConfigArgs(socketPath) {
 ...
 */
export function pruneDeadWakeTargets(targets, socketLiveness) {
    const out = {};
    let changed = false;
 ...
 */
export function wakeTargetChanged(existing, fresh) {
    return (!existing || existing.socketPath !== fresh.socketPath || existing.threadId !== fresh.threadId);
}
 ...
 */
export function wakeRetryBackoffMs(attempts, jitter = 0) {
...
export function wakeRetryExpired(firstEnqueuedAt, now, maxAgeMs = WAKE_RETRY_MAX_AGE_MS) {
...
//# sourceMappingURL=codex-wake-resolve.js.map
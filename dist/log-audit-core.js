export const LOG_AUDIT_NUDGE = (count) => `Heads up: ${count}+ state-changing tool calls since the last \`borg_log\` post. ` +
 ...
 */
export function evaluateLogAudit(entries, renderNudge = (count) => `Heads up: ${count}+ state-changing tool calls since the last \`borg_log\` post. ` +
    'If that work was a substantive unit (a change that ships, a blocker hit, a finding ' +
    "worth sharing), post to the cube log per your role's conventions before continuing. " +
    'Choose `to: "broadcast"` or a non-empty selector array; never omit `to`.') {
    ...
    const threshold = 3;
    const maxScan = 400;
    const role = (entry) => entry?.type ?? entry?.role ?? entry?.info?.role;
    const parts = (entry) => {
    ...
    const isUserPrompt = (entry) => {
        ...
        return Array.isArray(content) && content.some((part) => part?.type === 'text' && typeof part.text === 'string' && part.text.trim().length > 0);
    };
    ...
    if (index >= 0 && isUserPrompt(entries[index]))
        index--;
    let material = 0;
    let scanned = 0;
    for (; index >= 0 && scanned < maxScan; index--, scanned++) {
        ...
        const payloadTool = payload?.type === 'function_call' || payload?.type === 'custom_tool_call'
            ? payload.name
            : null;
        if (typeof payloadTool === 'string') {
            if (logTools.has(payloadTool))
                return material >= threshold ? renderNudge(material) : null;
            if (materialTools.has(payloadTool))
                material++;
            if (material >= threshold)
                return renderNudge(material);
        }
        ...
        for (let partIndex = content.length - 1; partIndex >= 0; partIndex--) {
            ...
            if (!name)
                continue;
            if (logTools.has(name))
                return material >= threshold ? renderNudge(material) : null;
            if (materialTools.has(name))
                material++;
            if (material >= threshold)
                return renderNudge(material);
        }
...
//# sourceMappingURL=log-audit-core.js.map
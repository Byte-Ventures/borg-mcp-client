...
function isValidSeatRecord(ref, value) {
    ...
    if (r.bindingOrder !== undefined &&
        (typeof r.bindingOrder !== 'number' || !Number.isSafeInteger(r.bindingOrder) || r.bindingOrder < 1))
        return false;
    ...
    if (r.commonDir !== undefined && !isNonEmptyString(r.commonDir))
        return false;
    if (r.repositoryOrigin !== undefined && !isNonEmptyString(r.repositoryOrigin))
        return false;
    ...
    if (r.state === 'active') {
        ...
        if (r.sessionId !== undefined || r.bindingOrder !== undefined)
            return false;
    }
...
function parseStore(raw) {
    ...
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        ...
        if (candidate.version === SEATS_VERSION &&
            ...
            for (const [ref, record] of Object.entries(seats)) {
                if (record !== null &&
                    ...
                    const exactReplacement = replacement !== null &&
                        ...
                        SEAT_BEARER_RE.test(replacement.credential);
            ...
            const store = { version: SEATS_VERSION, seats: seats };
 ...
 */
function repairOrderedDuplicateBindings(store) {
    const byWorktree = new Map();
    for (const entry of Object.entries(store.seats)) {
        const [, record] = entry;
        if (record.state !== 'active' || record.worktree === undefined)
            continue;
...
export class AmbiguousSeatBindingError extends Error {
    constructor(worktree) {
        super(`Borg found multiple active seats for worktree ${worktree}, but their bind order is unavailable. ` +
...
export async function activateAndBindSeat(input) {
    ...
    return withStore(SEATS_FILE, emptyStore, parseStore, async (txn) => {
        ...
        if (digestOf(record.credential) !== input.expectedPendingDigest)
            return 'replaced';
        const highestOrder = Object.values(txn.data.seats).reduce((highest, candidate) => Math.max(highest, candidate.bindingOrder ?? 0), 0);
        ...
        await txn.commit();
        for (const retiredRef of retiredRefs)
            rejectedSeatRefs.delete(retiredRef);
        rejectedSeatRefs.delete(ref);
        return 'activated';
    });
}
...
export async function getActiveSeatForWorktree(worktree) {
    ...
    const candidates = Object.entries(store.seats)
        .filter(([ref, record]) => record.state === 'active' && record.worktree === worktree && seatRef(record) === ref);
    if (candidates.length > 1)
        throw new AmbiguousSeatBindingError(worktree);
    return candidates[0]?.[1] ?? null;
}
/** Remember a definitive server auth or eviction verdict in this process only. */
export function markSeatRejected(ref) {
    if (REF_RE.test(ref))
        rejectedSeatRefs.add(ref);
}
/**
...
export async function hasActiveSeatInDifferentCloneFamily(cubeId, repositoryOrigin, commonDir) {

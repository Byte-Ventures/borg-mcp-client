...
export function parseLaunchSeatArgs(args) {
    let target;
    let cube;
    ...
    if (!target)
        return { ok: false, error: 'requires a drone label or id prefix' };
    return { ok: true, target, ...(cube ? { cube } : {}) };
}
function safeRealpath(deps, path) {
    try {
...
function seatKey(cubeId, droneId) {
...
export async function readLocalSeatRows(deps) {
    ...
    return rows.sort((a, b) => a.droneLabel.localeCompare(b.droneLabel)
...
export function formatLocalSeatRows(rows) {
    ...
    const widths = headings.slice(0, -1).map((heading, index) => Math.max(heading.length, ...values.map((row) => row[index].length)));
    const render = (row) => row.slice(0, -1).map((value, index) => value.padEnd(widths[index] + 2)).join('') + row[row.length - 1];
    return `${render(headings)}\n${values.map(render).join('\n')}\n`;
}
function formatAmbiguousMatches(rows) {
    ...
    return rows.map((row) => `  ${row.droneLabel.padEnd(labelWidth + 2)}${row.cubeName.padEnd(cubeWidth + 2)}id:${row.droneId.slice(0, 8)}`).join('\n');
}
export async function runSeats(deps) {
    try {
...
export async function runLaunchSeat(args, deps) {
    let rows;
    try {
    ...
    if (matches.length > 1) {
        deps.stderr(`borg launch: '${args.target}' matches ${matches.length} drones:\n` +
...
//# sourceMappingURL=seat-commands.js.map
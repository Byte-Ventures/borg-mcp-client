...
import { resolveGitRepositoryContext, getOrCreateRepositoryIdentity, } from './repository-identity.js';
...
function isConnectionReset(error) {
    ...
    for (let depth = 0; depth < 2; depth += 1) {
        ...
        if (typed.code === 'ECONNRESET')
            return true;
        candidate = typed.cause;
    }
    return false;
}
function unreadLogTransportFailure(cause) {
    return new BorgServerUnreachableError('Borg could not complete the unread log read after one automatic retry. ' +
        'The request may have reached the server; repeat `borg_read-log unread_only=true` until caught up.', { cause });
}
...
async function localCubeComposition(active) {
    ...
    if (!drone || !role)
        throw new Error('Local Borg server no longer recognizes this drone');
    return {
...
async function authedFetch(path, init = {}) {
    const { droneSession, apiUrl, authToken, serverTrustIdentity: suppliedTrustIdentity, localSessionCredentialRef, retryMode, headers, ...rest } = init;
    if (apiUrl === undefined) {
        throw new Error('Selected Borg server authority state is missing or unreadable');
    }
    ...
    const requestWithRetry = async () => {
        try {
            ...
            try {
                ...
                if (isConnectionReset(retryError))
                    throw unreadLogTransportFailure(retryError);
                throw retryError;
            }
        }
    };
    ...
    if (!response.ok) {
        ...
        throw new BorgServerHttpError(response.status, serverMessage
            ...
            : `Borg server request failed (HTTP ${response.status})${retryGuidance}`, code);
    }
    return response;
}
/**
...
export async function createCube(name, cubeDirective, opts, connection) {
    ...
    if (!repoContext) {
        throw new Error('Local Borg server cube creation must run inside a git repository — the cube is homed to the caller\'s repository. Run borg_create-cube from a repository worktree.');
    }
    ...
    if (created.result === 'resolved') {
        throw new Error(`A cube already exists for this repository (id: ${created.cube_id}). The local Borg server homes one cube per repository; use borg_assimilate to join it or borg_update-cube to change its settings.`);
    }

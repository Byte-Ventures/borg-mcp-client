    ...
    if (args.cubeName !== undefined) {
        ...
        if (matches.length > 1) {
            const list = matches
                ...
                .join('\n');
            return {
                error: `'${args.cubeName}' is ambiguous — ${matches.length} cubes in cubes.json share that name:\n${list}\n` +
...
export async function runLaunchAll(args, deps, opts = {}) {
    ...
    if (discovered.length === 0) {
        if (args.flags.only !== undefined) {
            ...
            deps.stdout(`No worktrees found for cube '${cubeName}' — have you run \`borg assimilate --worktree\` to create any drone seats?\n`);
        }
        return 0;
    }
    ...
    for (const c of lockLaunchable) {
        ...
        if (status === 'frozen') {
            deps.stderr(`skipping ${c.droneLabel} (${c.worktreeDir}): seat frozen (subscription downgrade) — ` +
    ...
    if (launchable.length === 0) {
        deps.stdout(`All ${lockLaunchable.length} discovered drone(s) for cube '${cubeName}' have evicted/frozen seats; nothing to launch.\n`);
        return 0;
    }
    // 5. --dry-run

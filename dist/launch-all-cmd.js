...
function checkTmuxAvailable(deps) {
    try {
        deps.runSync('tmux', ['-V']);
        return true;
    }
...
async function resolveTargetCube(args, deps) {
    if (args.cubeName !== undefined) {
        ...
        if (matchesByCubeId.size > 1) {
            const list = [...matchesByCubeId.entries()]
                ...
                .join('\n');
            return {
                error: `'${args.cubeName}' is ambiguous — ${matchesByCubeId.size} distinct cubes on this machine share that name:\n${list}\n` +
    ...
    if (!active) {
        ...
        const candidates = (await Promise.all(knownCubeIds.map((cubeId) => discoverDroneCandidates({ targetCubeId: cubeId }, deps)))).flat();
        const candidatesByCubeId = new Map();
        ...
        if (candidatesByCubeId.size === 0) {
            return {
                error: 'no saved drone worktrees were found for this repository; pass a cube name explicitly as ' +
                    '`borg launch-all <cube-name>`, or cd into a drone worktree and rerun `borg launch-all`',
            };
        }
        return {
            error: `linked drone worktrees for this repository belong to ${candidatesByCubeId.size} cubes; ` +
...
function resolveAttachMode(args, deps) {
    if (args.flags.noAttach)
        return 'none';
    if (!deps.isTTY())
        return 'none';
    if (deps.getEnv('TMUX'))
...
function printCheatSheet(sessionName, deps) {
...
function sanitizeSessionName(cubeName) {
...
async function reconcileRoster(deps, seat, launchStartISO, launchedDroneIds, now = Date.now, sleep = (ms) => new Promise((r) => setTimeout(r, ms))) {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        try {
            ...
            deps.stderr(`roster confirmation skipped (${tokenRotated ? 'token rotated mid-launch' : reason}); ` +
        ...
        if ([...result.values()].every((v) => v === 'verified'))
            break;
        await sleep(500);
    }
    return result;
...
export function resolveLaunchDelayMs(flag, env) {
    if (flag !== undefined && Number.isInteger(flag) && flag >= 0)
        return flag;
...
export async function runLaunchAll(args, deps, opts = {}) {
    ...
    if (discovered.length === 0) {
        if (args.flags.only !== undefined) {
            ...
            if (!looksLikeDroneLabel(args.flags.only)) {
                deps.stderr(`note: --only '${args.flags.only}' is matched best-effort by drone label; role-name matching ` +
                    `needs a drone session and is not available here.\n`);
            }
        }
        else {
            deps.stdout(`No worktrees found for cube '${cubeName}' — have you run \`borg assimilate --worktree\` to create any drones?\n`);
        }
        return 0;
    }
    // 4. live-drone skip (lock-file), unless --force
    const lockLaunchable = [];
    ...
    for (const c of lockLaunchable) {
        ...
        if (status === 'evicted') {
            evictedCount += 1;
            deps.stderr(`skipping ${c.droneLabel} (${c.worktreeDir}): drone no longer in cube (evicted) — ` +
        ...
        if (status === 'unreachable') {
            deps.stderr(`note: could not reach ${c.droneLabel}'s server to confirm that drone (network/timeout) — launching anyway.\n`);
        }
            ...
            deps.stderr(`note: ${c.droneLabel}'s server returned an error while confirming that drone (transient) — launching anyway.\n`);
        }
        else if (status === 'indeterminate') {
            deps.stderr(`note: could not confirm ${c.droneLabel} is live (network/transient) — launching anyway.\n`);
        }
        launchable.push(c);
    }
    ...
    if (args.flags.dryRun) {
        ...
        for (const c of launchable)
            deps.stdout(`  ${c.droneLabel}  ${c.worktreeDir}\n`);
        return 0;
    }
    // 6. >6 confirmation
